...
import { Activity, Anchor, Clock, Database, Gauge, Globe, Layers, Plug, Scale, Server, ShieldAlert, ShieldCheck } from 'lucide-react';
...
function OverviewTab({ ship, period, meta, qualityValue, qualityStatus }: { ship: NonNullable<ShipDashboardResponse['data']>; period: string; meta: ShipDashboardResponse['meta']; qualityValue: string | null; qualityStatus: MetricStatus }) {
    ...
    return <>
        <div className="grid-cards">
            <MetricCard icon={<Plug size={15} />} title="Management VPN" value={vpnStatus} period="Current state" source="Ship dashboard connectivity" freshness={freshnessLabel(meta)} status={statusClass(vpnStatus)} description="Connectivity state is not a traffic counter." />
            <MetricCard icon={<Server size={15} />} title="Managed devices" value={deviceTotal} unit="devices" period="Current inventory" source="Inventory database" freshness={freshnessLabel(meta)} status="healthy" description={deviceOnline === null ? 'Online count not returned.' : `${deviceOnline} online`} />
            <MetricCard icon={<Gauge size={15} />} title="Data quality" value={qualityValue} unit={qualityValue ? 'of 100%' : ''} period={period} source="Telemetry quality metadata" freshness={freshnessLabel(meta)} status={qualityStatus} description="Score is null when sources are missing." />
            <MetricCard icon={<Activity size={15} />} title="Traffic telemetry" value={ship.data_status === 'AVAILABLE' ? 'Available' : 'Chưa có dữ liệu'} period={period} source="Interface counters + RADIUS + IPFIX" freshness={freshnessLabel(meta)} status={ship.data_status === 'AVAILABLE' ? 'healthy' : 'unknown'} description="No /1024 conversion is applied; byte counters must come from the API." />
        </div>
...
function SummaryReconciliation({ data, period }: { data: NonNullable<ReconciliationResponse['data']>; period: string }) {
    ...
    return <>
        ...
        <div className="grid-cards"><MetricCard icon={<Scale size={15} />} title="CREW download gap" value={formatBytes(gaps?.crew_download_gap_bytes).value} unit={formatBytes(gaps?.crew_download_gap_bytes).unit} period={period} source="Reconciliation engine" freshness="Response metadata" status={gaps?.crew_download_gap_bytes === null || gaps?.crew_download_gap_bytes === undefined ? 'unknown' : 'warning'} /><MetricCard icon={<Scale size={15} />} title="CREW upload gap" value={formatBytes(gaps?.crew_upload_gap_bytes).value} unit={formatBytes(gaps?.crew_upload_gap_bytes).unit} period={period} source="Reconciliation engine" freshness="Response metadata" status={gaps?.crew_upload_gap_bytes === null || gaps?.crew_upload_gap_bytes === undefined ? 'unknown' : 'warning'} /><MetricCard icon={<Scale size={15} />} title="WAN download gap" value={formatPercent(gaps?.wan_download_gap_pct)} unit="" period={period} source="Reconciliation engine" freshness="Response metadata" status={gaps?.wan_download_gap_pct === null || gaps?.wan_download_gap_pct === undefined ? 'unknown' : 'warning'} /><MetricCard icon={<Scale size={15} />} title="WAN upload gap" value={formatPercent(gaps?.wan_upload_gap_pct)} unit="" period={period} source="Reconciliation engine" freshness="Response metadata" status={gaps?.wan_upload_gap_pct === null || gaps?.wan_upload_gap_pct === undefined ? 'unknown' : 'warning'} /></div>
...
function CrewTab({ users, radiusHealth, rawAccounting, sessions, selectedUsername, loading, error, onRetry, onLoadSessions }: { users?: CrewUsersResponse; radiusHealth?: CrewRadiusHealthResponse; rawAccounting?: CrewRawAccountingResponse; sessions?: CrewSessionsResponse; selectedUsername: string; loading: boolean; error: string; onRetry: () => void; onLoadSessions: (username: string) => void }) {
    ...
    return <>
        ...
        <div className="grid-cards">
            <MetricCard icon={<ShieldCheck size={15} />} title="RADIUS health" value={radius?.status ?? 'Chưa có dữ liệu'} period="Current state" source={radius?.source ?? 'RADIUS accounting'} freshness={freshnessLabel(radiusHealth?.meta)} status={statusClass(radius?.status)} description={`${formatCount(radius?.healthy_endpoints)} / ${formatCount(radius?.configured_endpoints)} healthy endpoints`} />
            <MetricCard icon={<ShieldAlert size={15} />} title="RADIUS HA" value={radius?.ha_compliant === undefined ? null : radius.ha_compliant ? 'Compliant' : 'Not compliant'} period="Current state" source="RADIUS service registry" freshness={freshnessLabel(radiusHealth?.meta)} status={radius?.ha_compliant ? 'healthy' : 'warning'} description={`Minimum required: ${formatCount(radius?.min_required_endpoints)}`} />
            <MetricCard icon={<Clock size={15} />} title="Accounting freshness" value={radius?.accounting_freshness_seconds === null || radius?.accounting_freshness_seconds === undefined ? null : radius.accounting_freshness_seconds} unit={radius?.accounting_freshness_seconds === null || radius?.accounting_freshness_seconds === undefined ? '' : 'seconds'} period="Current state" source="RADIUS accounting" freshness={freshnessLabel(radiusHealth?.meta)} status={radius?.accounting_freshness_seconds === null || radius?.accounting_freshness_seconds === undefined ? 'unknown' : radius.accounting_freshness_seconds > 300 ? 'warning' : 'healthy'} />
            <MetricCard icon={<Globe size={15} />} title="Service/domain usage" value={data?.service_usage_capability?.status === 'AVAILABLE' ? 'Available' : 'Chưa có dữ liệu'} period={formatPeriod(data?.period)} source="DNS classifier (NetFlow + DNS log)" freshness={freshnessLabel(users?.meta)} status={data?.service_usage_capability?.status === 'AVAILABLE' ? 'healthy' : 'unknown'} description={isRecord(data?.service_usage_capability) && typeof data.service_usage_capability.message === 'string' ? data.service_usage_capability.message : 'Null service_usage/domain_usage remains unavailable.'} />
            <MetricCard icon={<Scale size={15} />} title="NetFlow vs RADIUS coverage" value={formatPercent(billing?.coverage_pct)} period={formatPeriod(data?.period)} source="NetFlow (measured) vs RADIUS (billing)" freshness={freshnessLabel(users?.meta)} status={billing === null || billing === undefined ? 'unknown' : 'healthy'} description={billing ? `NetFlow ${formatBytes(billing.netflow_total_bytes).value} ${formatBytes(billing.netflow_total_bytes).unit} of RADIUS ${formatBytes(billing.billing_total_bytes).value} ${formatBytes(billing.billing_total_bytes).unit} — raw ratio, not auto-scaled.` : 'No RADIUS sessions to compare against this period.'} />
        </div>
        <section className="glass-panel dashboard-section">
...
function TrafficFlowTab({ flows, unknownFlows, rawRecords, loading, error, onRetry }: { flows?: BusinessFlowsResponse; unknownFlows?: BusinessRawRecordsResponse; rawRecords?: BusinessRawRecordsResponse; loading: boolean; error: string; onRetry: () => void }) {
    ...
    return <>
        ...
        <div className="grid-cards"><MetricCard icon={<Layers size={15} />} title="Flow count" value={formatCount(data?.flow_count)} unit="flows" period={formatPeriod(data?.period)} source={data?.source ?? 'IPFIX_FLOW'} freshness={freshnessLabel(flows?.meta)} status="healthy" /><MetricCard icon={<Database size={15} />} title="Total bytes" value={formatBytes(data?.total_bytes).value} unit={formatBytes(data?.total_bytes).unit} period={formatPeriod(data?.period)} source={data?.source ?? 'IPFIX_FLOW'} freshness={freshnessLabel(flows?.meta)} status={data?.total_bytes === null || data?.total_bytes === undefined ? 'unknown' : 'healthy'} /><MetricCard title="Unknown classification" value={data?.unknown_reason ?? 'Chưa có dữ liệu'} period={formatPeriod(data?.period)} source="BUSINESS flows" freshness={freshnessLabel(flows?.meta)} status={data?.unknown_reason ? 'warning' : 'unknown'} /></div>
        <BusinessClassificationSections flows={flows} />
        <BusinessRawSections unknownFlows={unknownFlows} rawRecords={rawRecords} />
    </>;
}
